...
//     console.log("self.innerWidth", self.innerWidth, self.innerHeight)
  ...
  function zoomToImage(d, duration) {
    ...
    if (isInIframe) translateNow[0] = -scale * (d.x - padding) - (max * 0.25) / 2 + margin.left;
  ...
  function showDetail(d) {        
    ...
    detailContainer.classed("hide", isInIframe).classed("sneak", utils.isMobile() || isInIframe);
  ...
  if (!isInIframe) d3.select(".searchbar").classed("hide", false);
  ...
  function loadMiddleImage(d) {
    ...
    if (config.loader.textures.detail.csv) {
      ...
      url = config.loader.textures.detail.url + d.id + ".avif";
    }
  ...
  function loadBigImage(d) {
    ...
    if (config.loader.textures.big.csv) {
      ...
      url = config.loader.textures.big.url + d.id + page + ".avif";
    }
